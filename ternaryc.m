function [hd,x,y]=ternaryc(c1,c2,c3,d,symbol)
...
% Plot the points
hold on
%XX = zeros(1,length(c1));
%YY = zeros(1,length(c1));

for i=1:length(c1)    
    x(i)=0.5-c1(i)*cos(pi/3)+c2(i)/2;
    y(i)=0.866-c1(i)*sin(pi/3)-c2(i)*cot(pi/6)/2;
    if marker == 1
        hd(i)=plot(x(i),y(i),'db','markerfacecolor','b');
    else
        in=round((d(i)-miv)*(length(map)-1)/(mav-miv));
        if in==0;in=1;end
        if in > length(map);in=length(map);end
        hd(i)=plot(x(i),y(i),symbol,'color',map(in,:),'markerfacecolor',map(in,:));
    end
